...
/**
 * Verify the passcode entered on the admin dashboard login. The passcode is
 * sourced entirely from the environment; if it is not configured, no input
 * can match and the dashboard stays locked.
 */
export function verifyAdminPasscode(input: string): boolean {
  return env.ADMIN_DEV_PASSCODE.length > 0 && input === env.ADMIN_DEV_PASSCODE;
}
