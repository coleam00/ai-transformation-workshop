...
async function getStatusStats(status: PollStatus) {
  const [liveCount, allTimeCount] = await Promise.all([
    countPollsByStatus(status),
    getLegacyCount(status),
  ]);
  return { status, liveCount, allTimeCount };
}

export default async function AdminPage() {
  const stats = await Promise.all(STATUSES.map(getStatusStats));
    ...
    <div className="relative min-h-screen bg-zinc-50 font-sans dark:bg-zinc-950">
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-6 py-16">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {stats.map(({ status, liveCount, allTimeCount }) => (
            <Card key={status}>
              <CardHeader>
                <CardTitle className="capitalize">{status}</CardTitle>
                <CardDescription>Live count</CardDescription>
              </CardHeader>
